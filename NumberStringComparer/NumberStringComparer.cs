...
namespace NumberStringComparer {
	...
	{
		public static HashSet<Type> allowedPropertyTypes = new HashSet<Type> { typeof(string), typeof(int), typeof(double), typeof(float), typeof(decimal), typeof(long), typeof(short), };
		...
		public static bool IsComplexType(Type type) {
			return !type.IsPrimitive && type != typeof(string) && type != typeof(decimal) && !IsKeyValuePair(type);
		}
		...
		public struct NumberString<U> : IComparable<NumberString<U>> 
		{
			...
			public double? Number {
				get => number;
				private set { } //do nothing, readonly
			}
			private string text;
			public string Text {
				get => text;
			...
			public static NumberString<U> Parse(object value) {
				...
				if (IsComplexType(type)) {
					throw new InvalidOperationException($"Cannot call this version of {nameof(Parse)} with complex object (Type = {value.GetType()}). Use the other version passing and object instance and propertyName");
				}
				if (IsKeyValuePair(type)) {		//normally CompareTo just sends the key from KVP but just in case, we handle it
			...
			public int CompareTo(NumberString<U> other) {
			...
			public static IReadOnlyList<string> GetParts(string text) {
				...
				if (text.Contains(',')) return text.Split(new [] { ',' }, StringSplitOptions.RemoveEmptyEntries)	//must check this before double.TryParse because that ignores commas
					.Select(s => s.Trim())
